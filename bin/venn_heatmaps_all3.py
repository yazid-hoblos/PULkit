...
Arguments:
    ...
    -o/--output      Output directory for plots (default: report/)
...
def main():
    ...
    parser.add_argument("-o", "--output", default="report", help="Output directory for plots")
    ...
    for f in projection_files:
        print(f"Loading {f}")
        projection_dfs.append(load_cgc_finder(f))
    projection_df = pd.concat(cgc_dfs, ignore_index=True)
    ...
    merged = merge_datasets(cgc_df, cazy_df, dbcan_df)
    ...
    venn_path = os.path.join(args.output, "overlap_venn_all.png")
...
# dbcan_dir = "easy-dbcan-results/s__Prevotella_denticola"

# output_dir = "plots"
# os.makedirs(output_dir, exist_ok=True)

print("Loading all CGC files from projection directory...")
cgc_files = glob.glob(os.path.join(cgc_dir, "*.tsv"))  # Just .tsv files in this folder
cgc_dfs = []
for f in cgc_files:
    print(f"Loading {f}")
    cgc_dfs.append(load_cgc_finder(f))
cgc_df = pd.concat(cgc_dfs, ignore_index=True)

print("Loading CAZY curated data...")
cazy_df = load_cazy_curated(cazy_path)

print("Loading all dbCAN files...")
dbcan_files = glob.glob(os.path.join(dbcan_dir, "*-cgc-output", "cgc_standard.out"))
dbcan_dfs = []
for f in dbcan_files:
    print(f"Loading {f}")
    dbcan_dfs.append(load_dbcan(f))
...
venn_path = "report/overlap_venn_all.png"
print(f"Saving Venn diagram to {venn_path}")
plot_venn(overlaps, venn_path)
...
for sys1, sys2 in pairs:
    ...
    heatmap_path = 'report/heatmap_{sys1}_vs_{sys2}.png'
    print(f"Saving heatmap to {heatmap_path}")
    plt.figure(figsize=(10,8))
    plot_system_overlap_heatmap(merged, sys1, sys2, heatmap_path.format(sys1=sys1, sys2=sys2))
