...
def load_projections(file_path):
    ...
    df['cgc_system'] = df['cgc_system'].astype(str)
...
def plot_system_overlap_heatmap(merged_df, sys_col1, sys_col2, output_path):
    ...
    plt.xlabel(sys_col2)
    plt.ylabel(sys_col1)
    # sys_col1 = sys_col1.replace('CAZy', 'Empirical')
    # sys_col2 = sys_col2.replace('CAZy', 'Empirical')
...
def plot_system_overlap_heatmap_combined_norm(merged_df, sys_col1, sys_col2, output_path):
    matched = merged_df.dropna(subset=[sys_col1, sys_col2])

    # Create sets of genes per system
    def system_to_genes(df, system_col):
        return {
            system: set(df[df[system_col] == system][['contig', 'start', 'stop', 'gene_id']].apply(tuple, axis=1))
            for system in df[system_col].dropna().unique()
        }

    genes_sys1 = system_to_genes(matched, sys_col1)
    genes_sys2 = system_to_genes(matched, sys_col2)

    jaccard_matrix = pd.DataFrame(index=genes_sys1.keys(), columns=genes_sys2.keys())

    for s1 in genes_sys1:
        for s2 in genes_sys2:
            g1 = genes_sys1[s1]
            g2 = genes_sys2[s2]
            intersection = len(g1 & g2)
            union = len(g1 | g2)
            # jaccard = intersection / union if union > 0 else 0
            jaccard = intersection / max(len(g1), len(g2))
            jaccard_matrix.loc[s1, s2] = jaccard

    jaccard_matrix = jaccard_matrix.fillna(0).astype(float)

    plt.figure(figsize=(10,8))
    sns.heatmap(jaccard_matrix, cmap='Purples', cbar_kws={'label': 'Jaccard Index (shared / union)'})
    plt.xlabel(sys_col2)
    plt.ylabel(sys_col1)
    plt.title(f'System overlap (normalized by union): {sys_col1} vs {sys_col2}')
    plt.tight_layout()
    plt.savefig(output_path, dpi=600, bbox_inches='tight')
    plt.show()
    plt.close()
...
def main():
    ...
    dbcan_files = glob.glob(os.path.join(args.dbcan, "*-cgc-output", "cgc_standard_out.tsv"))
    if not dbcan_files:
        dbcan_files = glob.glob(os.path.join(args.dbcan, "*", "cgc_standard_out.tsv"))
    dbcan_dfs = []
    ...
    print("Filtering PANORAMA and dbCAN genes to only those from genomes present in CAZy...")
    valid_contigs = set(cazy_df['contig'].unique())
    projection_df = projection_df[projection_df['contig'].isin(valid_contigs)]
    dbcan_df = dbcan_df[dbcan_df['contig'].isin(valid_contigs)]
    ...
    for sys1, sys2 in pairs:
        ...
        # plot_system_overlap_heatmap_combined_norm(merged, sys1, sys2, heatmap_path)
