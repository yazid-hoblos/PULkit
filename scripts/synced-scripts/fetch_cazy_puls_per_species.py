...
def write_to_csv(csv_filename, pul_list):
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as csvfile:
        ...
        for pul_name, species, pul_url in pul_list:
            ...
            # time.sleep(1) # Uncomment to avoid overwhelming the server
            protein_str = ";".join(proteins)  # Join all protein IDs with semicolon 
...
def main():
    ...
    if pul_list:
        ...
        write_to_csv(f"{species_input.replace(' ', '_')}_cazy_puls.csv", pul_list)
