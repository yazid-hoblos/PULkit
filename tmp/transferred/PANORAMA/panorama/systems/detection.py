...
import threading
# Global lock for this specific list
_combinations_lock = threading.Lock()
...
def get_subcombinations(combi: Set[GeneFamily], combinations: List[FrozenSet[GeneFamily]]
    ...
    # with _combinations_lock:
    # to_remove = [c for c in combinations if c.issubset(combi)]
    # for c in to_remove:
...
def search_unit_in_cc(graph: nx.Graph, system_gf: Set[GeneFamily], func_unit: FuncUnit, source: str,
    ...
    for cc in sorted([cc for cc in nx.connected_components(graph) if len(cc) >= func_unit.min_total],
        ...
        # if "PGY10_RS05280" in [gf.name for gf in cc]:
        #     print("Found PGY10_RS05280 in connected component:", cc)
        #     print(gfs_in_cc)
        #     print(check_for_families(gfs_in_cc, gf2fam, fam2source, func_unit) )
        #     matrix,_,_ = get_gfs_matrix_combination(gfs_in_cc, gf2fam, fam2source)
        #     print(check_needed_families(matrix, func_unit))
        # print(f"CC: {len(cc)}, GFs in CC: {gfs_in_cc}")
        if not gfs_in_cc:
            continue
        # print("check", cc-gfs_in_cc)
        check, gf2meta_info = check_for_families(gfs_in_cc, gf2fam, fam2source, func_unit) #cc
        # matrix,_,_ = get_gfs_matrix_combination(gfs_in_cc, gf2fam, fam2source)
        # check = check_needed_families(matrix, func_unit) 
        if check:
            new_unit = SystemUnit(functional_unit=func_unit, source=source, gene_families=cc,
                                  families_to_metainfo=gf2meta_info)
            # print(f"New unit found: {cc}", len(cc))
            is_subset = False
            for unit in sorted(detected_su, key=len, reverse=True):
                if new_unit.is_subset(unit):
                    # print(f"Unit {new_unit} is a subset of {unit}, merging them.")
                    unit.merge(new_unit)
                    is_subset = True
            if not is_subset:
...
def search_unit_in_combination(graph: nx.Graph, families: Set[GeneFamily],
    ...
    accessory_gfs = {gf for gf2metadata in accessory_gfs2metadata.values() for gf in gf2metadata.keys() if
                     gf in families}
    # print("Check:", set(mandatory_gfs | accessory_gfs)==set(families))
    # with _lock:
    while len(combinations) > 0:
        ...
        # print(f"Context combination: {[gf.name for gf in context_combination]}")
        organisms_of_interest = combinations2orgs[context_combination]
        context_combination = set(context_combination)
        context_gfs_name = {gf.name for gf in context_combination}  & {gf.name for gf in mandatory_gfs | accessory_gfs} #& {gf.name for gf in graph.nodes}
        # print(set([gf.name for gf in context_combination])==context_gfs_name)
        if len(context_gfs_name) >= func_unit.min_total:
            ...
            if check_needed_families(filtered_matrix, func_unit):
                ...
                # export_graph_for_gephi(local_graph, "local.gexf")
                ...
                # print("Nodes removed:", len(node2remove))
                # print("Nodes left:", len(local_graph.nodes))
                ...
                # print("Nodes after filtering:", len(local_graph.nodes))
                # export_graph_for_gephi(local_graph, "local_unfiltered_t1.gexf")
                # exit(0)
                logging.getLogger('PANORAMA').debug('search unit in filtered context')
                # print the families in local graph but not in context_combination
                # print("Families in local graph but not in context_combination:", set(local_graph.nodes) - context_combination)
...
def search_unit_in_context(graph: nx.Graph, families: Set[GeneFamily], gene_fam2mod_fam: Dict[GeneFamily, Set[Family]],
    ...
    """
    detected_su: Set[SystemUnit] = set()
    # print("Before everything:", detected_su)
    ...
    for cc_graph in [graph.subgraph(cc).copy() for cc in sorted(nx.connected_components(graph),
        ...
        # if "PGY10_RS05280" in [gf.name for gf in cc_graph.nodes]:
        #     print("Found PGY10_RS05280 in connected component:", cc_graph.nodes)
        families_in_cc = families_in_context.copy()
        neutral_families = neutral_families_in_context.copy()
        families_in_cc &= cc_graph.nodes
        # print("Found",len(cc_graph.nodes), "nodes in cc")
        # print(f"None Model Families")
        # print("Non-model",len(cc_graph.nodes - families_in_context))
        # print([gf.name for gf in families_in_cc])
        ...
        if len(families_in_cc) > 0:
            ...
            combinations_in_cc = get_subcombinations(families_in_cc, combinations) # cc_graph.nodes
            # print(f"Combinations in cc: {len(combinations_in_cc)}")
            # print("Combinations after filtering:", len(combinations))
            # for comb in combinations_in_cc:
                # print([gf.name for gf in comb])
            # exit(0)
            if len(combinations_in_cc) > 0:
                ...
                # print(f"New detected system unit: {detected_su}")
                ...
                for new_su in new_detected_su:
                    ...
                    for unit in sorted(detected_su, key=len, reverse=True):
                        if new_su.is_subset(unit):
                            # print(f"WARNING: Unit {[f for f in new_su.families]} is a subset of {[f for f in unit.families]}, merging them.")
                            is_subset = True
                            unit.merge(new_su)
                    if not is_subset:
    ...
    # check if any unit is subset of another unit in detected_su
    for u1 in sorted(detected_su, key=len, reverse=True):
        for u2 in sorted(detected_su, key=len, reverse=True):
            if u1 != u1 and u1.is_subset(u2):
                print(f"WARNING: Unit {[f for f in u1.families]} is a subset of {[f for f in u2.families]}")
                break
    return detected_su
...
import threading
_detected_su_lock = threading.Lock()
    ...
    for func_unit in model.func_units:
        ...
        if len(fu_families) >= func_unit.min_total:
            ...
            if check_needed_families(matrix, func_unit):
                ...
                # combinations2orgs = defaultdict(set)
                detected_su: Set[SystemUnit] = set()
                combinations = sorted([c for c in combinations2orgs.keys() if len(c) >= func_unit.min_total], key=lambda fs: (len(fs), sorted(fs)), reverse=True)

                print("Searching for system units in context started.")
                for comb in combinations:
                    # for _,_,data in cc.edges(data=True):
                    #     print(data)
                    orgs = {v[0] for v in combinations2orgs[comb]}
                    merged_graph = nx.compose_all([v[1] for v in combinations2orgs[comb]])

                    new_cc = filter_local_context(context.subgraph(merged_graph).copy(), orgs)
                    # print(list(nx.isolates(new_cc)))
                    to_remove = [gf for sub_cc in nx.connected_components(new_cc) 
                                 if not check_needed_families(matrix[[gf.name for gf in sub_cc.intersection(gene_families)]],func_unit) 
                                 for gf in sub_cc]
                    # print(f"Removing {len(to_remove)} nodes from {len(new_cc.nodes)} nodes")
                    new_cc.remove_nodes_from(to_remove)
                    # combinations2orgs[frozenset(combo.intersection(new_cc.nodes))] = {gf for gf in new_cc.nodes}

                    # filtered_matrix = matrix[[gf.name for gf in comb.intersection(new_cc.nodes)]]
                    # print(filtered_matrix)
                    check, gf2meta_info = check_for_families(comb.intersection(new_cc.nodes), gf2fam, fam2source, func_unit)
                    if check: #and check_needed_families(filtered_matrix, func_unit):
                        # print(f"Searching unit in combination: {[gf.name for gf in comb]}")
                        new_su = SystemUnit(functional_unit=func_unit, source=source,
                                            gene_families={gf for gf in new_cc.nodes}, families_to_metainfo= gf2meta_info) 

                        # with _detected_su_lock:               
                        #     is_subset = False
                        #     for unit in detected_su: #sorted(detected_su, key=len, reverse=True):
                        #         if new_su.is_subset(unit):
                        #             is_subset = True
                        #             unit.merge(new_su)
                        #             # break
                        ...
                        if not any([new_su.is_subset(su) for su in detected_su]):
                            detected_su.add(new_su)
                            
                # print(f"Found {len(detected_su)} units in combination.")
                # print("CHECK",[fam.name for fam in set(list(detected_su)[0].families)])
                
                # export the context to a graph file
                # export_graph_for_gephi(context, f"context_graph_t1.gexf")
...
def search_system(model: Model, gene_families: Set[GeneFamily], gf2fam: Dict[GeneFamily, Set[Family]],
    ...
    # print(f"System units found for model {model.name}: {su_found}")
    ...
    if check_for_needed_units(su_found, model) and not check_for_forbidden_unit(su_found, model):
        if len(su_found) == 1:  # only one functional unit found so each one correspond to a system
            for fu in next(iter(su_found.values())):
                # print(f"Functional unit found: {fu}")
...
def launch(args):
    ...
    #                              need_info=need_info, sources=args.source, max_workers=args.threads, lock=lock,
    #                              disable_bar=args.disable_prog_bar)
    ...
    # output = args.output if args.output else Path("pangenome_systems")
    # force = args.force
    # for pangenome in tqdm(pangenomes, total=len(pangenomes), unit='pangenome', disable=args.disable_prog_bar):
    #     pangenome_res_output = mkdir(output / f"{pangenome.name}", force=force)
    #     fam_index = pangenome.compute_org_bitarrays()
    #     pangenome_proj, organisms_proj = project_pangenome_systems(pangenome, system_source=args.source,fam_index=fam_index)
    #     source_res_output = mkdir(pangenome_res_output / f"{system_source}", force=force)
    #     write_projection_systems(source_res_output, pangenome_proj, organisms_proj)
...
def parser_detection(parser):
    ...
    optional.add_argument("-o", "--output", required=False, type=Path, default=None)
