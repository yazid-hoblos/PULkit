...
def filter_local_context(graph: nx.Graph, organisms: Set[Organism],
    ...
    for f1, f2, data in graph.edges(data=True):
        ...
        # if f1.name == "HMPREF9137_RS03580" or f2.name == "HMPREF9137_RS03580":
        #     print(f"Edge between {f1.name} and {f2.name} has Jaccard proportions: "
        #           f"f1_proportion={f1_proportion}, f2_proportion={f2_proportion}")


        # print(f1_proportion, f2_proportion)
        # Add the edge to the new graph if it meets the Jaccard threshold
        if f1_proportion >= jaccard_threshold and f2_proportion >= jaccard_threshold:
            # Add the edge with the updated edge data
            new_graph.add_edge(f1, f2, **data)
        # else:
        #     print(f"Edge between {f1.name} and {f2.name} filtered out due to low Jaccard index: ")
...
def check_for_families(gene_families: Set[GeneFamily], gene_fam2mod_fam: Dict[GeneFamily, Set[Family]],
    ...
    def fam_sort_key(item):
        ...
        if family.presence == "mandatory" and family.name not in mandatory_seen:
            ...
            presence_rank = 2  # always lowest priority
        else:
            presence_rank = 3  # already seen mandatory/accessory
        return (-score, presence_rank, family.name)  # negative for descending
    ...
    for gf, fam2meta_info in sorted(gf2fam2meta_info.items(), key=lambda x: len(x[1])):
        sorted_fam2meta_info = sorted(fam2meta_info.items(), key=fam_sort_key) # key = lambda x: x[1][2], reverse=True
        add = False
    ...
    if found_forbidden:
        ...
        # print(f"Mandatory seen: {mandatory_seen}, Accessory seen: {accessory_seen}")
